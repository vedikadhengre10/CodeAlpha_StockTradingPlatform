    ...
    if (state.user.alerts) {
        ...
        state.user.alerts.forEach(a => {
            ...
            if (stock) {
                ...
                if (hit) {
                    a.status = "TRIGGERED";
                    a.message = `ALERT: ${a.symbol} hit $${formatMoney(stock.currentPrice)} (target: $${formatMoney(a.targetPrice)} ${a.alertType === "ABOVE" ? "▲" : "▼"})`;
                ...
                if (fill) {
                    ...
                    if (o.type === "BUY") {
                        ...
                        if (state.user.cashBalance >= finalCost) {
                            ...
                            showToast(`📈 Filled LIMIT BUY: ${o.quantity} ${o.symbol} @ $${formatMoney(stock.currentPrice)}`, "success");
                            stateChanged = true;
                        } else {
                        ...
                        if (holding && holding.quantity >= o.quantity) {
                            ...
                            showToast(`📈 Filled LIMIT SELL: ${o.quantity} ${o.symbol} @ $${formatMoney(stock.currentPrice)}`, "success");
                            stateChanged = true;
                        }
                    }
                } else {
    ...
    if (state.isOfflineSandbox) {
        state.stocks.forEach(s => {
            if (s.status === "ACTIVE") {
                ...
                const change = (Math.random() * 2 - 1) * volatility * s.currentPrice;
...
async function renderTransactionsHistory() {
    if (state.isOfflineSandbox) {
        document.getElementById("tx-commission-total").textContent = `Commission Paid: $${formatMoney(state.user.transactions.filter(t => t.commission).reduce((sum, t) => sum + t.commission, 0))}`;
...
async function triggerAccountAction(actionType) {
    if (state.isOfflineSandbox) {
        ...
        if (actionType === "save") {
            ...
            content += `Available Cash: $${formatMoney(state.portfolioData.cashBalance)}\n`;
            content += `Portfolio Value: $${formatMoney(state.portfolioData.totalValue)}\n\n`;
            content += `=== ACTIVE HOLDINGS ===\n`;
            state.portfolioData.holdings.forEach(h => {
                content += `${h.symbol} (${h.companyName}): ${h.quantity} shares @ avg $${formatMoney(h.avgCost)} (Current: $${formatMoney(h.currentPrice)}) | Value: $${formatMoney(h.currentValue)}\n`;
            });
            ...
            content += `Cash Available:      $${formatMoney(state.portfolioData.cashBalance)}\n`;
            content += `Stock Holdings:      $${formatMoney(state.portfolioData.stockValue)}\n`;
            content += `Net Portfolio worth: $${formatMoney(state.portfolioData.totalValue)}\n`;
            ...
            content += `Commission Paid:     $${formatMoney(state.user.transactions.reduce((sum, t) => sum + (t.commission || 0), 0))}\n`;
...
async function renderLeaderboard() {
    if (state.isOfflineSandbox) {
        const board = [
            ...
            { username: state.user.username, tier: state.user.tier, totalValue: state.portfolioData ? state.portfolioData.totalValue : state.user.cashBalance, netPnL: state.user.netPnL }
        ];
...
async function handleWithdraw(e) {
    ...
    if (state.isOfflineSandbox) {
        if (amount > 0 && state.user.cashBalance >= amount) {
            state.user.cashBalance -= amount;
            saveSandboxUserState();
            ...
            showToast("Invalid amount or insufficient cash balance.", "error");
        }
        return;
    }
